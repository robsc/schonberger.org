...
So for a little while now, i've been encouraging people in the team to always use Double Checked Locking in java. Whats that you may ask? Well, in Java, getting a synchronized lock on things can be slow. A lot of code, including code in {% highlight java %} java.util.* {% endhighlight %} has points like this: 
