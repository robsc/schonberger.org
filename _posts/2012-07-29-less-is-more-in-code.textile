...
{% endhighlight %}
Fairly simple; There's only one real way of writing our code, even though using the else, we can see that, yes, there was a roundabout way of doing this. In <A href="http://golang.org">Go</a>, we have the same thing, basically, except we don't have to use as many brackets. We can write, instead:
{% highlight go %}
if a {
	b
}
...
Looking at how to do all this in Ruby, and we find a different way. Ruby allows many more ways of writing the same expression; There's two reasons for this:
<ol>
	<li> In addition to if, there's <em>unless</em>, which is just an "if not" shorthand</li>
